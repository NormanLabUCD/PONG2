...
validatePlink() {
    ...
    mkdir -p "$output/tmp" || die "Cannot create output directory"

    awk '$1 == 19 && $4 >= 55235681 && $4 <= 55378697 {print}' "$prefix.bim" > "$output/tmp/KIR_region.txt" || die "No SNPs found in the KIR chromosome 19 region"

    awk 'NR==FNR {ref[$1":"$2]=1; next}
     {pos_key=$1":"$4}
     pos_key in ref' "$PACKAGE_ROOT/extdata/hg19/kirhg19_pos.txt" "$output/tmp/KIR_region.txt" > "$output/tmp/matched_snps.txt"

    matched_count=$(wc -l < "$output/tmp/matched_snps.txt")
    ref_count=$(wc -l < "$PACKAGE_ROOT/extdata/hg19/kirhg19_pos.txt")
